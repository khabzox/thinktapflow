...
export async function generateContent(formData: FormData) {
  try {
    ...
    const needsMonthlyReset =
      !profile.monthly_reset_date || profile.monthly_reset_date !== currentMonthStart;
    ...
    if (
      tierLimits.daily_generations !== -1 &&
      profile.daily_usage_count >= tierLimits.daily_generations
    ) {
      throw new GenerationError(
    ...
    const result: GenerationResult = {
      posts: Array.isArray(generatedArray)
        ...
        : {},
      metadata:
        Array.isArray(generatedArray) && generatedArray[0]?.metadata
