...
export function useGenerations() {
  ...
  const deleteGenerations = async (ids: string[]) => {
    try {
      ...
      const { data, error } = await supabase.from("generations").delete().in("id", ids).select(); // Add select to see what was deleted
    ...
    try {
      ...
      setGenerations(prev => {
        const updatedGenerations = prev.map(gen =>
          ids.includes(gen.id) ? { ...gen, status: "archived" as const } : gen,
        );
  ...
  const unarchiveGenerations = async (ids: string[]) => {
    try {
      ...
      setGenerations(prev => {
        const updatedGenerations = prev.map(gen =>
          ids.includes(gen.id) ? { ...gen, status: "completed" as const } : gen,
        );
        return updatedGenerations;
      });
  ...
  const regenerateContent = async (id: string) => {
    try {
      ...
      // Update local state
      setGenerations(prev =>
        prev.map(gen => (gen.id === id ? { ...gen, posts: result.data!.posts as any } : gen)),
      );
