...
export function PostContentDialog({ isOpen, onClose, generation }: PostContentDialogProps) {
    ...
    const postsByPlatform = generation.posts.reduce((acc, post) => {
    ...
    const handleExportCSV = () => {
        try {
            const rows = [
                ['Platform', 'Variation', 'Content', 'Hashtags', 'Mentions']
            ];
    ...
    const handleExportJSON = () => {
        try {
            const data = {
                ...
                content: generation.platforms.reduce((acc, platform) => {
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-3xl h-[80vh] flex flex-col overflow-hidden">
                <DialogHeader className="flex-shrink-0">
                    ...
                    <DialogDescription>
                        Generated content for {generation.platforms.length} platform{generation.platforms.length !== 1 ? 's' : ''}
                    </DialogDescription>
                </DialogHeader>

                <Tabs defaultValue={selectedPlatform} className="flex-1 flex flex-col min-h-0" onValueChange={setSelectedPlatform}>
                    <TabsList className="grid grid-cols-3 lg:grid-cols-6 h-auto gap-4 flex-shrink-0">
                                                        ...
                                                        <div className="flex flex-wrap gap-1">
                                                            {post.mentions.map((mention, i) => (
                                                                <Badge key={i} variant="outline" className="text-blue-500">
                                                                    {getMentionPrefix(platform)}{mention}
                                                                </Badge>
                                                            ))}
                                                        </div>
                                                    )}
