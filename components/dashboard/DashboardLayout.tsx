  ...
  return (
    <Sidebar className="border-r-0" collapsible="icon">
        ...
        <div className="flex items-center gap-2 py-2">
          <div
            className={`flex h-8 w-8 items-center justify-center rounded-lg transition-all duration-200 hover:scale-110 ${
              isAdmin ? "bg-red-600" : "bg-primary"
            }`}
          >
            <Image
              src="/logo/logosaas-dark.png"
                ...
                return (
                  <SidebarMenuItem key={item.name}>
                    ...
                    >
                      <Link
                        href={item.href}
                        className="flex items-center gap-3 rounded-lg px-3 py-2 transition-all duration-200 hover:bg-sidebar-accent/80"
                      >
      ...
      <SidebarFooter className="border-t border-sidebar-border">
        <div className="flex items-center gap-3 py-3 transition-all duration-200 hover:cursor-pointer hover:rounded-lg hover:bg-sidebar-accent/80 sm:px-4 sm:group-data-[collapsible=icon]:px-0">
          ...
          <div className="min-w-0 flex-1 group-data-[collapsible=icon]:hidden">
            <p className="truncate text-sm font-medium">{isAdmin ? "Admin User" : "John Doe"}</p>
            <div className="flex items-center gap-1 truncate text-xs text-muted/70">
              <span
                className={`inline-block h-2 w-2 rounded-full ${isAdmin ? "bg-red-500" : "bg-green-500"}`}
              ></span>
    ...
    <SidebarProvider defaultOpen={true}>
      <div className="flex h-screen w-full">
        ...
        <SidebarInset className="flex-1">
          <header
            className={`flex h-16 items-center gap-4 border-b px-6 ${
              isAdmin
          ...
          >
            <div className="flex items-center gap-2">
              <SidebarTrigger className="transition-colors hover:bg-popover-foreground hover:text-accent-foreground" />
              <div className="hidden items-center rounded-md bg-muted/50 px-2 py-1 text-xs text-secondary-foreground md:flex">
                <kbd className="px-1 py-0.5 font-mono text-xs">Ctrl</kbd>
                <span className="mx-1">+</span>
                <kbd className="px-1 py-0.5 font-mono text-xs">B</kbd>
              </div>
            </div>
            <div className="flex-1">
              <div className="relative max-w-md">
                ...
                <Input
                  ...
                  className="focus-visible:popover-foreground/20 border-0 bg-muted/50 pl-9 transition-all duration-200 focus-visible:ring-1"
                />
              </div>
            </div>
            {isAdmin && (
